...
def test_basic_ts_plot(ts_vector = None) -> None:
    ...
    # Plot time series
    basic_plot_ts(ts_vector = ts_vector, title_str = 'Basic Time Series Plot', ylabel = 'y', lw = 3.0, ylim = None, xlabel = 'x', ax = ax)
...
def test_overlaid_ts_plot(normalized_ts_dict = None) -> None:
    ...
    overlaid_ts(normalized_ts_dict = normalized_ts_dict, title_str = 'Overlaid Time Series Plot', ylabel = 'y', xlabel = 'x', fontsize = 30, xticks = None, ylim = None, DEFAULT_ALPHA = 1.0, legend_present = True, DEFAULT_MARKERSIZE = 15, delete_yticks = False, ax = ax)

    # Save the plot
    save_loc = os.path.join(example_plot_folder_loc,"lineplots", 'overlaid_ts_plot.png')
    save_fig(fig,save_loc,dpi = 600)
...
if __name__ == '__main__':
    ...
    normalized_ts_dict = {'x': {'xvec': np.arange(0, 10, 0.1), 'ts_vector': x_data, 'lw': 3.0, 
                                'linestyle': '-', 'color': 'b'}, 'y': {'xvec': np.arange(0, 10, 0.1), 
                                'ts_vector': y_data, 'lw': 3.0, 'linestyle': '-', 'color': 'r'}, "sin": {'xvec': np.arange(0, 10, 0.1),"ts_vector": sin_data, "lw": 3.0, "linestyle": "-", "color": "g","zorder": 2},
                                  "cos": {'xvec': np.arange(0, 10, 0.1),"ts_vector": cos_data, "lw": 3.0, "linestyle": "-", "color": "k","zorder": 10}}
