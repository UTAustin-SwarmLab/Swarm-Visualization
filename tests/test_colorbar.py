...
from swarm_visualizer.utility import (
    save_fig,
    set_plot_properties,
    create_colorbar,
)
...
_DATA_FRAME = pd.DataFrame(
    {
        "x": _X_DATA,
        "y": _Y_DATA,
        "discrete_numerical_label": _DISCRETE_NUMERICAL_LABEL_DATA,
        "discrete_category_label": _DISCRETE_CATEGORY_LABEL_DATA,
        "continuous_label": _CONTINUOUS_LABEL_DATA,
    }
)

_DISCRETE_CATEGORY_VISIBLE_LABEL_DATA = [
    "Group 1",
    "Group 5",
    "Group 6",
    "Group 9",
...
@pytest.mark.parametrize(
    ("x_data", "y_data", "discrete_numerical_label_data", "discrete_palette"),
    [(_X_DATA, _Y_DATA, _DISCRETE_NUMERICAL_LABEL_DATA, _DISCRETE_PALETTE)],
)
def test_basic_discrete_colorbar(
    x_data, y_data, discrete_numerical_label_data, discrete_palette
) -> None:
    ...
    colors = [
        palette_colors[unique_values.tolist().index(i)]
        for i in discrete_numerical_label_data
    ]
    ...
    create_colorbar(
        ax[1], unique_values, palette=discrete_palette, discrete=True
    )

    # Save the plot
    save_loc = os.path.join(
        _SAVE_LOC, "colorbars", "discrete_colorbar_with_all_labels.png"
    )
...
def test_discrete_colorbar_w_limited_visible(
    x_data,
    y_data,
    discrete_category_label_data,
    discrete_category_visible_label_data,
    ...
    # Find colors for each value in the discrete numerical label data
    colors = [
        discrete_colors[unique_values.index(i)]
    ...
    create_colorbar(
        ax[1],
        unique_values,
        visible_labels=discrete_category_visible_label_data,
    ...
    # Save the plot
    save_loc = os.path.join(
        _SAVE_LOC, "colorbars", "discrete_colorbar_with_some_labels.png"
    )
    save_fig(fig, save_loc, dpi=600)


@pytest.mark.parametrize(
    ("data_frame", "x_var", "y_var", "hue", "continuous_palette"),
    [(_DATA_FRAME, "x", "y", "continuous_label", _CONTINUOUS_PALETTE)],
)
def test_basic_continous_colorbar(
    data_frame, x_var, y_var, hue, continuous_palette
) -> None:
    """Tests basic discrete colorbar.
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            "y",
            "continuous_label",
            _CONTINUOUS_VISIBLE_LABEL_DATA,
            _CONTINUOUS_PALETTE,
        )
    ],
)
def test_continous_colorbar_w_limited_visible(
    data_frame, x_var, y_var, hue, continuous_visible, continuous_palette
) -> None:
    """Tests basic discrete colorbar.
    ...
    create_colorbar(
        ...
        data_frame[hue],
        title="Continuous Colorbar",
        visible_labels=continuous_visible,
        palette=continuous_palette,
    ...
    # Save the plot
    save_loc = os.path.join(
        _SAVE_LOC, "colorbars", "continous_limited_data_colorbar.png"
    )
    plt.tight_layout()
    save_fig(fig, save_loc, dpi=600)
